...
Class salesController Extends baseController {
    public function index() {
        ...
        if ($_SESSION['role_logined'] != 1 && $_SESSION['role_logined'] != 3 && $_SESSION['role_logined'] != 4 && $_SESSION['role_logined'] != 8 && $_SESSION['role_logined'] != 7 && $_SESSION['role_logined'] != 2) {
            return $this->view->redirect('user/login');
        }
        ...
        $staff = $staff_model->getAllStaff(array('where'=>'( create_time <= '.strtotime($ketthuc).' )'),array('table'=>'new_salary, sales','where'=>'staff.staff_id = new_salary.staff  AND create_time <= sales_create_time'));
        $staff_data = array();
        ...
        if ($_SESSION['role_logined'] == 4) {
            if ($staff_model->getStaffByWhere(array('account'=>$_SESSION['userid_logined']))) {
                ...
                $staff = $staff_model->getAllStaff(array('where'=>'(staff_id='.$id_staff.' AND ( create_time <= '.strtotime($ketthuc).' ) )'),array('table'=>'new_salary, sales','where'=>'staff.staff_id = new_salary.staff AND create_time <= sales_create_time'));
                $staff_data = array();
                foreach ($staff as $staff) {
                    $staff_data['staff_id'][date('m-Y',$staff->create_time)][$staff->staff_id] = $staff->staff_id;
                    $staff_data['staff_name'][date('m-Y',$staff->create_time)][$staff->staff_id] = $staff->staff_name;
                ...
                $staff2 = $staff_model->getAllStaff(array('where'=>'(staff_id!='.$id_staff.' AND ( create_time <= '.strtotime($ketthuc).' ) )'),array('table'=>'new_salary, sales','where'=>'staff.staff_id = new_salary.staff AND create_time <= sales_create_time'));
