...
//var_dump(strtotime('28-09-2014'));
        ...
        if (isset($id) && $id > 0) {
            ...
            if ($trangthai==1) {
                $query .= ' AND receive_date > '.strtotime($ngaybatdau).' AND receive_date < '.strtotime($ngayketthuc);
            }
            else{
                $query .= ' AND ( expect_date < '.strtotime($batdau).' OR ( expect_date > '.strtotime($ngaybatdau).' AND expect_date < '.strtotime($ngayketthuc).') ) AND (pay_money is null OR pay_money != receivable.money)';
            }
        ...
        $data = array(
            'where' => 'money < money_in AND money_in != 0 ',
        );
