...
Class profitController Extends baseController {
    public function index() {
        ...
        $this->view->data['title'] = 'Tổng lợi nhuận';
        ...
        foreach ($sales as $sale) {
            $sale_data['doanhthu'] = isset($sale_data['doanhthu'])?($sale_data['doanhthu']+$sale->revenue+$sale->revenue_vat+$sale->other_revenue+$sale->other_revenue_vat):($sale->revenue+$sale->revenue_vat+$sale->other_revenue+$sale->other_revenue_vat);
            $sale_data['chiphi'] = isset($sale_data['chiphi'])?($sale_data['chiphi']+$sale->cost+$sale->cost_vat):($sale->cost+$sale->cost_vat);
            
            $financial_data['doanhthu'] = isset($financial_data['doanhthu'])?($financial_data['doanhthu']+$sale->cost):(0+$sale->cost);
        }

        $other_cost = $assets_model->queryAssets('SELECT * FROM assets, costs WHERE assets.costs=costs.costs_id AND (sale_estimate=1 OR trading_estimate=1) AND assets_date >= '.strtotime($batdau).' AND assets_date <= '.strtotime($ketthuc));
        ...
        foreach ($agents as $agent) {
            ...
            $financial_data['doanhthu'] = isset($financial_data['doanhthu'])?($financial_data['doanhthu']+$agent->total_cost+$agent->bill_cost-(($agent->cost_17*160000)+($agent->cost_18*40000)+$agent->document_cost+$agent->pay_cost)):(0+$agent->total_cost+$agent->bill_cost-(($agent->cost_17*160000)+($agent->cost_18*40000)+$agent->document_cost+$agent->pay_cost));
        ...
        foreach ($manifests as $agent) {
            ...
            $financial_data['doanhthu'] = isset($financial_data['doanhthu'])?($financial_data['doanhthu']+($agent->cost_sg+$agent->cost_cm+$agent->driver_cost+$agent->commission_cost+$agent->other_cost+$agent->other_vendor_cost)):(0+($agent->cost_sg+$agent->cost_cm+$agent->driver_cost+$agent->commission_cost+$agent->other_cost+$agent->other_vendor_cost));
        ...
        $financial_data['doanhthu'] = isset($financial_data['doanhthu'])?round($financial_data['doanhthu']*0.1):0;
    ...
    public function sale(){
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Lợi nhuận sale';
        ...
        $other_cost = $assets_model->queryAssets('SELECT * FROM assets, costs WHERE assets.costs=costs.costs_id AND sale_estimate=1 AND assets_date >= '.strtotime($batdau).' AND assets_date <= '.strtotime($ketthuc));
        foreach ($other_cost as $cost) {
            if($cost->check_pay==1){
                $sale_data['phichuyentien'] = isset($sale_data['phichuyentien'])?($sale_data['phichuyentien']+str_replace('-', "", $cost->total)):(0+str_replace('-', "", $cost->total)); 
            }
    ...
    public function trading(){
        ...
        if ($_SESSION['role_logined'] != 1 && $_SESSION['role_logined'] != 3 && $_SESSION['role_logined'] != 7 && $_SESSION['role_logined'] != 4 && $_SESSION['role_logined'] != 2 && $_SESSION['role_logined'] != 8 && $_SESSION['role_logined']!=10) {
            return $this->view->redirect('user/login');
        }
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Lợi nhuận trading';
        ...
        $sale_model = $this->model->get('salereportModel');
        $sale_vendor_model = $this->model->get('salevendorModel');
        $assets_model = $this->model->get('assetsModel');
    ...
    public function agent(){
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Lợi nhuận agent';
        ...
        $this->view->show('profit/agent');

    }

    public function tcmt(){
        $this->view->setLayout('admin');
        if (!isset($_SESSION['userid_logined'])) {
            return $this->view->redirect('user/login');
        }
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Lợi nhuận TCMT';
    ...
    public function financial(){
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Lợi nhuận tài chính';
        ...
        foreach ($sales as $sale) {
            $financial_data['doanhthu'] = isset($financial_data['doanhthu'])?($financial_data['doanhthu']+$sale->cost):(0+$sale->cost);
            $financial_data['lamhang'] = isset($financial_data['lamhang'])?($financial_data['lamhang']+$sale->cost+$sale->cost_vat):(0+$sale->cost+$sale->cost_vat);
            $financial_data['loinhuan'] = isset($financial_data['loinhuan'])?($financial_data['loinhuan']+$sale->revenue+$sale->revenue_vat):(0+$sale->revenue+$sale->revenue_vat);
        }
        ...
        foreach ($agents as $agent) {
            ...
            $financial_data['lamhang'] = isset($financial_data['lamhang'])?($financial_data['lamhang']+$agent->total_cost+$agent->bill_cost-(($agent->cost_17*160000)+($agent->cost_18*40000)+$agent->document_cost+$agent->pay_cost)):(0+$agent->total_cost+$agent->bill_cost-(($agent->cost_17*160000)+($agent->cost_18*40000)+$agent->document_cost+$agent->pay_cost));
            $financial_data['loinhuan'] = isset($financial_data['loinhuan'])?($financial_data['loinhuan']+$agent->total_offer):(0+$agent->total_offer);

           $phi_agent = $assets_model->queryAssets('SELECT * FROM obtain WHERE money=1100000 AND customer='.$agent->customer.' AND agent='.$agent->agent_id);
            foreach ($phi_agent as $phi) {
                $phidaily = $phi->money;
            }
            $financial_data['loinhuan'] = isset($financial_data['loinhuan'])?($financial_data['loinhuan']+$phidaily):(0+$phidaily);
        ...
        foreach ($manifests as $agent) {
           ...
           $financial_data['lamhang'] = isset($financial_data['lamhang'])?($financial_data['lamhang']+($agent->cost_sg+$agent->cost_cm+$agent->driver_cost+$agent->commission_cost+$agent->other_cost+$agent->other_vendor_cost)):(0+($agent->cost_sg+$agent->cost_cm+$agent->driver_cost+$agent->commission_cost+$agent->other_cost+$agent->other_vendor_cost));
           $financial_data['loinhuan'] = isset($financial_data['loinhuan'])?($financial_data['loinhuan']+$agent->revenue_vat+$agent->revenue):(0+$agent->revenue_vat+$agent->revenue);

        }


        $other_cost = $assets_model->queryAssets('SELECT * FROM assets, costs WHERE assets.costs=costs.costs_id AND check_invoice=1 AND (sale_estimate=1 OR agent_estimate=1 OR trading_estimate=1 ) AND assets_date >= '.strtotime($batdau).' AND assets_date <= '.strtotime($ketthuc));
        foreach ($other_cost as $cost) {
            $financial_data['chiphi'] = isset($financial_data['chiphi'])?($financial_data['chiphi']+str_replace('-', "", $cost->total)):(0+str_replace('-', "", $cost->total));
            
        }

        $other_cost = $assets_model->queryAssets('SELECT * FROM assets, costs WHERE assets.costs=costs.costs_id AND (sale_estimate=1 OR agent_estimate=1 OR trading_estimate=1 ) AND assets_date >= '.strtotime($batdau).' AND assets_date <= '.strtotime($ketthuc));
        foreach ($other_cost as $cost) {
            if($cost->check_invoice!=1){
               $financial_data['chiphikhac'] = isset($financial_data['chiphikhac'])?($financial_data['chiphikhac']+str_replace('-', "", $cost->total)):(0+str_replace('-', "", $cost->total));
            }
            
        }

        $financial_data['doanhthu'] = isset($financial_data['doanhthu'])?round($financial_data['doanhthu']*0.1)+round(($financial_data['loinhuan']-$financial_data['lamhang']-($financial_data['doanhthu']*0.1)-$financial_data['chiphikhac'])*0.1):0;

        $this->view->data['financial_data'] = $financial_data;
        $this->view->data['ngaytao'] = $ngaytao;
        $this->view->data['ngaytaobatdau'] = $ngaytaobatdau;

        $this->view->show('profit/financial');
