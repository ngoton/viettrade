                    ...
                    foreach ($vendor_cost as $v) {
                        ...
                        if ($sale_vendor_data['check_deposit'] == 1) {
                            if($sale_vendor->getVendorByWhere(array('trading'=>$_POST['yes'],'vendor'=>$v['vendor'],'type' => $v['cost_type']))){
                                ...
                                $old_invoice_cost = $sale_vendor->getVendorByWhere(array('trading'=>$_POST['yes'],'vendor'=>$v['vendor'],'type' => $v['cost_type']))->invoice_cost;
                                $old_pay_cost = $sale_vendor->getVendorByWhere(array('trading'=>$_POST['yes'],'vendor'=>$v['vendor'],'type' => $v['cost_type']))->pay_cost;
                                $old_document_cost = $sale_vendor->getVendorByWhere(array('trading'=>$_POST['yes'],'vendor'=>$v['vendor'],'type' => $v['cost_type']))->document_cost;
                                ...
                                if($payable->getCostsByWhere(array('money'=>$old_cost,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']))){
                                    if($sale_vendor_data['cost'] > 0){
                                        $check = $payable->getCostsByWhere(array('money'=>$old_cost,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']));

                                        if ($check->money >= $payable_data['money'] && $check->approve > 0) {
                                            $payable_data['approve'] = 10;
                                        }

                                        $payable->updateCosts($payable_data,array('money'=>$old_cost,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']));
                                    }
                                    else{
                                        $payable->queryCosts('DELETE FROM payable WHERE check_vat=0 AND money='.$old_cost.' AND vendor='.$sale_vendor_data['vendor'].' AND trading='.$_POST['yes'].' AND cost_type='.$sale_vendor_data['type']);
                                    }
                                }
                                else{
                                    if($sale_vendor_data['cost'] > 0){
                                ...
                                if($receivable->getCostsByWhere(array('money'=>$old_cost,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes'])))){
                                    if($sale_vendor_data['cost'] > 0){
                                        $receivable->updateCosts($receivable_data,array('money'=>$old_cost,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes'])));
                                    }
                                    else{
                                        $receivable->queryCosts('DELETE FROM receivable WHERE check_vat=0 AND money='.$old_cost.' AND vendor='.$sale_vendor_data['vendor'].' AND trading='.$_POST['yes']);
                                    }
                                }
                                else{
                                    if($sale_vendor_data['cost'] > 0){
                            ...
                            if($sale_vendor->getVendorByWhere(array('trading'=>$_POST['yes'],'vendor'=>$v['vendor'],'type' => $v['cost_type']))){
                                    ...
                                    if($payable->getCostsByWhere(array('money'=>$old_cost_vat,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']))){
                                        $check = $payable->getCostsByWhere(array('money'=>$old_cost_vat,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']));

                                        if ($check->money >= $payable_data['money'] && $check->approve > 0) {
                                            $payable_data['approve'] = 10;
                                        }

                                            $payable->updateCosts($payable_data,array('money'=>$old_cost_vat,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']));
                                        
                                    }
                                    elseif(!$payable->getCostsByWhere(array('money'=>$old_cost_vat,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']))){
                                    ...
                                    elseif(!$payable->getCostsByWhere(array('money'=>$old_cost,'vendor' => $sale_vendor_data['vendor'],'trading'=>trim($_POST['yes']),'cost_type' => $sale_vendor_data['type']))){
                                        if($sale_vendor_data['cost'] > 0){
                                            $payable->createCosts($payable_data);
                                        }
                                    }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['xoa'])) {
                ...
                return true;
                        ...
                        $pa = $payable->getAllCosts(array('where'=>'trading='.$_POST['yes']));
                        foreach ($pa as $p) {
                            $assets->queryAssets('DELETE FROM assets WHERE payable='.$p->payable_id);
                            $pay->queryCosts('DELETE FROM pay WHERE payable='.$p->payable_id);
                        }
