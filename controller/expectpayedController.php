...
Class expectpayedController Extends baseController {
    public function index() {
        ...
        if (isset($id) && $id > 0) {
            ...
            if ($trangthai==1) {
                $query .= ' AND pay.pay_date > '.strtotime($ngaybatdau).' AND pay.pay_date < '.strtotime($ngayketthuc);
            }
            else{
                $query .= ' AND ( expect_date < '.strtotime($batdau).' OR ( expect_date > '.strtotime($ngaybatdau).' AND expect_date < '.strtotime($ngayketthuc).') ) AND (pay_money is null OR pay_money != money)';
            }
        ...
        $data = array(
            'where' => '(money_in IS NULL OR money_in <= 0) AND money != 0',
        );
